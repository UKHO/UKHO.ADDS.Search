...
- [ ] Work Item 1: Replace the current custom compact-row renderer with a Studio-owned read-only `xterm.js` output surface that shows real Studio events end to end
  ...
  - [ ] Task 1.1: Introduce the read-only terminal host
    - [ ] Step 1: Add `xterm.js` and any required companion package(s) to `src/Studio/Server/search-studio/package.json`.
    - [ ] Step 2: Refactor `SearchStudioOutputWidget` so it hosts a terminal container rather than custom-rendered row markup.
    - [ ] Step 3: Initialize the terminal in read-only mode and ensure disposal occurs when the widget is torn down.
    - [ ] Step 4: Keep the panel clearly Studio-owned and non-terminal in interaction semantics despite the rendering primitive.
  - [ ] Task 1.2: Preserve the merged-stream output format
    - [ ] Step 1: Extend `search-studio-output-format.ts` with stable line serialization suitable for terminal output.
    - [ ] Step 2: Keep explicit visible `time`, `severity`, `source`, and `message` text on each line.
    - [ ] Step 3: Preserve chronological oldest-to-newest line order in the visible stream.
    - [ ] Step 4: Ensure source metadata remains present in the rendered text for future filtering readiness.
  - [ ] Task 1.3: Add baseline verification for the terminal host
    - [ ] Step 1: Add or update tests for stable merged-stream line serialization.
    - [ ] Step 2: Add focused verification for chronological append order in the output service.
    - [ ] Step 3: Add focused verification for terminal-host initialization/disposal behavior where practical without over-coupling to implementation details.
    - [ ] Step 4: Document a manual smoke path for confirming real Studio output appears in the read-only terminal surface.
  - **Files**:
...
- [ ] Work Item 2: Add terminal-compatible severity styling and reveal-latest behavior so the newest output stays visible in normal use
  ...
  - [ ] Task 2.1: Implement reveal-latest behavior for the terminal surface
    - [ ] Step 1: Choose the simplest terminal-host mechanism for keeping the newest line visible.
    - [ ] Step 2: Implement default reveal-latest behavior when new output is appended.
    - [ ] Step 3: Ensure clear resets the terminal buffer and reveal state cleanly.
    - [ ] Step 4: Confirm behavior remains correct with chronological output order.
  - [ ] Task 2.2: Add pastel severity treatment for terminal output
    - [ ] Step 1: Extend output-format helpers to emit terminal-compatible severity styling.
    - [ ] Step 2: Apply pastel blue `#A9C7FF` to `INFO` and pastel red `#FFB3BA` to `ERROR`.
    - [ ] Step 3: Keep the emphasis token-led so styling remains subtle and readable.
    - [ ] Step 4: Ensure the implementation remains theme-respecting and easy to adjust later.
  - [ ] Task 2.3: Add targeted verification for reveal-latest and severity formatting
    - [ ] Step 1: Add tests for reveal-latest helper logic or focused widget-level behavior.
    - [ ] Step 2: Add tests for terminal-compatible severity formatting helpers.
    - [ ] Step 3: Add verification for clear-followed-by-append behavior.
    - [ ] Step 4: Add manual smoke notes for continuous output review in the terminal surface.
  - **Files**:
...
- [ ] Work Item 3: Add `Copy all` and confirm the terminal-backed output remains naturally selectable and copy-friendly
  ...
  - [ ] Task 3.1: Confirm read-only selection and copy-friendliness
    - [ ] Step 1: Review terminal configuration for anything that impedes natural selection or text copy.
    - [ ] Step 2: Ensure the visible output remains readable and selectable in the terminal surface.
    - [ ] Step 3: Avoid introducing prompt-like affordances or fake command-entry behavior.
  - [ ] Task 3.2: Add `Copy all` toolbar action
    - [ ] Step 1: Introduce a dedicated toolbar command/contribution for `Copy all` alongside `Clear output`.
    - [ ] Step 2: Serialize the full merged output stream using the stable visible line format.
    - [ ] Step 3: Copy the merged stream text to clipboard without requiring selection.
    - [ ] Step 4: Keep command wiring aligned with the existing frontend module and output-command pattern.
  - [ ] Task 3.3: Add targeted interaction verification
    - [ ] Step 1: Add verification that the copied text preserves stable merged-stream ordering and source metadata.
    - [ ] Step 2: Add verification that `Copy all` is only exposed for `Studio Output`.
    - [ ] Step 3: Add manual smoke notes for selecting visible output text and using the toolbar copy action.
    - [ ] Step 4: Add focused verification that no channel-switch UI is introduced.
  - **Files**:
...
- [ ] Work Item 4: Harmonize the terminal-backed `Studio Output` baseline and document the accepted direction
  ...
  - [ ] Task 4.1: Review cross-surface consistency
    - [ ] Step 1: Confirm `Studio Output` aligns visually with the Studio shell panel and toolbar model.
    - [ ] Step 2: Confirm the surface remains clearly read-only despite its terminal-grade density.
    - [ ] Step 3: Confirm severity palette, merged stream, reveal-latest behavior, and `Copy all` feel coherent together.
  - [ ] Task 4.2: Complete verification and documentation
    ...
    - [ ] Step 3: Update `wiki/Tools-UKHO-Search-Studio.md` if the accepted terminal-backed baseline needs contributor guidance.
    - [ ] Step 4: Record any deferred density or metadata-display refinements separately.
  - **Files**:
