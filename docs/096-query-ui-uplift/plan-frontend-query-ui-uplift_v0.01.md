...
- [ ] Work Item 1: Deliver the single-screen raw-query workspace shell
  ...
  - [ ] Task 1: Reshape the `QueryServiceHost` page into the new single-screen shell
    - [ ] Step 1: Replace the current `Home.razor` three-column composition with a top command bar, left query-insight column, centre split workspace, right diagnostics column, and bottom detail-drawer host region.
    - [ ] Step 2: Add or repurpose host components so the layout is expressed as focused sections rather than nested cards.
    - [ ] Step 3: Move the old details experience out of the always-visible right column so the page no longer uses the current placeholder `DetailsPanel` pattern as the main diagnostic surface.
    - [ ] Step 4: Keep the page interactive with the existing Blazor Server rendering model.
    - [ ] Step 5: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 2: Add Monaco-based generated plan display in the centre-left pane
    - [ ] Step 1: Reuse the Monaco editor integration approach already established in `tools/RulesWorkbench/Components/JsonEditor.razor` and its JavaScript interop pattern.
    - [ ] Step 2: Introduce the Query host equivalent component and supporting static assets in `src/Hosts/QueryServiceHost`.
    - [ ] Step 3: Populate the editor with the generated query plan from the raw-query execution path.
    - [ ] Step 4: Keep the editor writable from the start so later edited-plan execution work can layer onto the same component without a second refactor.
    - [ ] Step 5: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 3: Extend host-local models and state so generated-plan data reaches the page
    - [ ] Step 1: Extend the host response model to carry the generated query plan in a form suitable for Monaco display.
    - [ ] Step 2: Update `QueryUiSearchClient` so the existing repository-owned query result is projected into the richer host response without moving planning logic into the host.
    - [ ] Step 3: Update `QueryUiState` to retain the latest generated plan separately from result-selection state.
    - [ ] Step 4: Keep raw-query search behaviour unchanged from the user perspective except for the new diagnostics shell.
    - [ ] Step 5: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 4: Flatten the results presentation and preserve selection
    - [ ] Step 1: Replace nested result cards with a simpler row-style list presentation.
    - [ ] Step 2: Preserve the selected-result visual state and click behavior.
    - [ ] Step 3: Keep empty, loading, and error states readable within the flatter layout.
    - [ ] Step 4: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 5: Add focused tests for the shell slice
    - [ ] Step 1: Add or extend host-state and host-composition tests in `test/QueryServiceHost.Tests` for generated-plan projection and shell state transitions.
    - [ ] Step 2: Add or extend query service tests only where the host contract now depends on additional result data from the application service.
    - [ ] Step 3: Avoid bUnit introduction; prefer repository-standard higher-level verification approaches, with browser automation added only if it can be introduced proportionately in this work package.
  - **Files**:
...
- [ ] Work Item 2: Add edited-plan execution, reset, and validation as a full vertical slice
  ...
  - [ ] Task 1: Extend the query application-service contract for supplied-plan execution
    - [ ] Step 1: Extend the repository-owned query application-service surface so callers can execute a supplied `QueryPlan` directly without duplicating execution logic in the host.
    - [ ] Step 2: Keep the Onion Architecture boundary intact by placing orchestration in `src/UKHO.Search.Services.Query` and execution in `src/UKHO.Search.Query` / `src/UKHO.Search.Infrastructure.Query`.
    - [ ] Step 3: Ensure the host continues to depend on the application-service abstraction rather than directly owning executor logic.
    - [ ] Step 4: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 2: Add host-local edited-plan execution workflow
    - [ ] Step 1: Extend the host client abstraction so `QueryUiState` can request execution of a supplied plan.
    - [ ] Step 2: Add state fields for generated-plan text, current editor text, validation errors, and last successful edited-plan execution.
    - [ ] Step 3: Wire the plan-pane `Search` and `Reset to generated plan` actions.
    - [ ] Step 4: Keep the raw-query and edited-plan actions visually distinct so the user understands which path is being executed.
    - [ ] Step 5: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 3: Implement validation and error projection
    - [ ] Step 1: Validate JSON parsing and query-plan contract deserialization before attempting execution.
    - [ ] Step 2: Surface blocking errors in the diagnostics area instead of replacing them with generic search failures.
    - [ ] Step 3: Preserve the current editor contents when validation fails so the user can correct the plan in place.
    - [ ] Step 4: Log invalid-plan execution attempts at the appropriate boundary with enough context for diagnostics.
    - [ ] Step 5: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 4: Add focused tests for edited-plan execution
    - [ ] Step 1: Add host tests covering reset behavior, validation-state transitions, and raw-query versus edited-plan action routing.
    - [ ] Step 2: Add or extend service tests covering supplied-plan execution through the application service.
    - [ ] Step 3: Add or extend infrastructure tests if executor or request-mapping behavior changes to support the edited-plan path.
  - **Files**:
...
- [ ] Work Item 3: Enrich diagnostics, metrics, and selected-result explanation
  ...
  - [ ] Task 1: Project rich diagnostics from the repository-owned query pipeline
    - [ ] Step 1: Extend the repository-owned query result shape as needed so the host can access final Elasticsearch request JSON, engine timing, and any warnings that belong in the diagnostics UI.
    - [ ] Step 2: Reuse existing query-plan content for extracted signals and transformation-trace derivation where possible rather than inventing duplicate host-only models.
    - [ ] Step 3: Keep Elasticsearch request generation owned by infrastructure so the host receives diagnostics rather than re-creating request JSON itself.
    - [ ] Step 4: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 2: Render left and right diagnostics columns in the host
    - [ ] Step 1: Populate the left column with extracted signals and a compact staged trace from raw input through plan and request.
    - [ ] Step 2: Populate the right column with stacked sections for request JSON, warnings/validation, and metrics.
    - [ ] Step 3: Keep the layout flat, using headings, spacing, and subtle separators instead of nested cards where possible.
    - [ ] Step 4: Preserve useful empty states so a fresh page load or failed execution does not collapse the layout.
    - [ ] Step 5: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 3: Add the lightweight bottom result-explain drawer
    - [ ] Step 1: Replace the current always-visible details pane behavior with a bottom drawer that opens on selected-result inspection.
    - [ ] Step 2: Show a lightweight explanation summary first, such as key matched fields, selected metadata, and raw detail where available.
    - [ ] Step 3: Keep deeper clause-level explain detail explicitly deferred unless backend support is already trivial to surface.
    - [ ] Step 4: Apply the repository documentation-pass standard from `./.github/instructions/documentation-pass.instructions.md` to all code written in this task.
  - [ ] Task 4: Add focused tests for diagnostics enrichment
    - [ ] Step 1: Add or extend service and infrastructure tests for enriched query result diagnostics and request-body capture.
    - [ ] Step 2: Add host tests covering diagnostics rendering state and selected-result drawer behavior.
    - [ ] Step 3: Add browser-level verification only if a lightweight repository-aligned harness is introduced without disproportionate overhead; otherwise record the targeted host/service verification used for this slice.
  - **Files**:
...
- [ ] Work Item 4: Complete the mandatory wiki review and record the work-package documentation outcome
  ...
  - [ ] Task 1: Review the query-related wiki reader path
    - [ ] Step 1: Review `wiki/Query-Pipeline.md` for the new split-screen developer workflow and edited-plan execution path.
    - [ ] Step 2: Review `wiki/Query-Walkthrough.md` for contributor-facing usage steps and screenshots or walkthrough prose if they materially help.
    - [ ] Step 3: Review `wiki/Query-Model-and-Elasticsearch-Mapping.md` for any terminology or request-visibility clarifications caused by the new diagnostics UI.
    - [ ] Step 4: Review `wiki/Glossary.md` if new terms such as transformation trace or edited query plan need explicit definitions.
  - [ ] Task 2: Record the final documentation outcome
    - [ ] Step 1: Update the relevant wiki pages, or explicitly record the grounded no-change result.
    - [ ] Step 2: Ensure the final work-package record states exactly which pages changed or were reviewed and left unchanged.
  - **Files**:
