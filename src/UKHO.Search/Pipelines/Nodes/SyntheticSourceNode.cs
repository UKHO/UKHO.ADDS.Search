...
namespace UKHO.Search.Pipelines.Nodes
{
	public sealed class SyntheticSourceNode<TPayload> : SourceNodeBase<Envelope<TPayload>>
	{
		...
		public SyntheticSourceNode(
			string name,
			ChannelWriter<Envelope<TPayload>> output,
			int messageCount,
			int keyCardinality,
			Func<int, TPayload> payloadFactory,
			Func<int, string>? keyFactory = null,
			Action<string>? log = null,
			IPipelineFatalErrorReporter? fatalErrorReporter = null)
			: base(name, output, log, fatalErrorReporter)
		{
			if (messageCount < 0)
			{
		...
		protected override async ValueTask ProduceAsync(ChannelWriter<Envelope<TPayload>> output, CancellationToken cancellationToken)
		{
			for (var i = 0; i < messageCount; i++)
			{
				...
				await WriteAsync(output, envelope, cancellationToken).ConfigureAwait(false);
			}
		}
	}
}
