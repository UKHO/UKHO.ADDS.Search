...
namespace UKHO.Search.Tests.Pipelines
{
	public sealed class MicroBatchTests
	{
		...
		public async Task Flushes_remaining_items_on_completion()
		{
			...
			var sourceToBatch = BoundedChannelFactory.Create<Envelope<int>>(capacity: 10, singleReader: true, singleWriter: true);
			var batchToSink = BoundedChannelFactory.Create<BatchEnvelope<int>>(capacity: 10, singleReader: true, singleWriter: true);

			var source = new SyntheticSourceNode<int>(
				"source",
				sourceToBatch.Writer,
				messageCount: 3,
				keyCardinality: 1,
				payloadFactory: i => i,
				fatalErrorReporter: supervisor);

			var batch = new MicroBatchNode<int>(
				"microbatch",
				partitionId: 0,
				sourceToBatch.Reader,
				batchToSink.Writer,
				maxItems: 10,
			...
			batches[0].Items.Count.ShouldBe(3);
			batches[0].Items.Select(i => i.Payload).ToArray().ShouldBe(new[] { 0, 1, 2 });
		}
		...
		public async Task Flushes_when_max_delay_is_reached()
		{
			...
			var batches = await ReadAllAsync(batchToSink.Reader, cts.Token);
			batches.Count.ShouldBe(1);
			batches[0].PartitionId.ShouldBe(7);
			batches[0].Items.Count.ShouldBe(1);
			batches[0].Items[0].Payload.ShouldBe(1);
			(batches[0].FlushedUtc - batches[0].CreatedUtc).ShouldBeGreaterThanOrEqualTo(maxDelay - TimeSpan.FromMilliseconds(50));
		}

		private static async Task<List<BatchEnvelope<int>>> ReadAllAsync(
			ChannelReader<BatchEnvelope<int>> reader,
