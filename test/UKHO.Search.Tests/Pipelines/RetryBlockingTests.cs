...
namespace UKHO.Search.Tests.Pipelines
{
	public sealed class RetryBlockingTests
	{
		...
		public async Task Retry_blocks_lane_in_order_and_dead_letters_after_exhaustion()
		{
			...
			var deadLetterFilePath = Path.Combine(Path.GetTempPath(), "ukho-search", Guid.NewGuid().ToString("N"), "retry-deadletter.jsonl");

			var graph = RetryPipelineGraph.Create(
				messageCount: 5,
				capacity: 10,
				retryPolicy,
				sinkDelay: null,
			...
			var payloads = graph.Sink.Items.Select(e => e.Payload).ToArray();
			...
			envelope.GetProperty("Payload").GetInt32().ShouldBe(2);
			envelope.GetProperty("Attempt").GetInt32().ShouldBe(3);
