	...
	public sealed class DeadLetterTests
	{
		...
		public async Task Poison_message_is_routed_to_dead_letter_and_pipeline_continues()
		{
			...
			var graph = HelloPipelineGraph.Create(
				...
				cts.Token,
				keyFactory: i => i == poisonIndex ? string.Empty : $"key-{i % keyCardinality}",
				deadLetterFilePath: filePath);
			...
			var sinkPayloads = graph.Sinks.SelectMany(s => s.Items).Select(e => e.Payload).ToArray();
			sinkPayloads.Length.ShouldBe(messageCount - 1);
			sinkPayloads.Max().ShouldBe(messageCount - 1);
			...
			root.GetProperty("Envelope").GetProperty("Payload").GetInt32().ShouldBe(poisonIndex);
			root.GetProperty("Error").GetProperty("Code").GetString().ShouldBe("KEY_EMPTY");
		}
	}
}
