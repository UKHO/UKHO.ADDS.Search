...
namespace UKHO.Search.Tests.Pipelines
{
	public sealed class MicroBatchMaxItemsTests
	{
		...
		public async Task Flushes_when_max_items_is_reached_and_does_not_exceed_batch_size()
		{
			...
			batches[0].Items.Select(i => i.Payload).ToArray().ShouldBe(new[] { 0, 1 });
			batches[1].Items.Select(i => i.Payload).ToArray().ShouldBe(new[] { 2 });
			batches[0].Items.Count.ShouldBe(2);
			batches[1].Items.Count.ShouldBe(1);
		}
