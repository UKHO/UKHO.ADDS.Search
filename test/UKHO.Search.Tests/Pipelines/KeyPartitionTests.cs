...
namespace UKHO.Search.Tests.Pipelines
{
	public sealed class KeyPartitionTests
	{
		...
		public async Task Same_key_always_routes_to_the_same_lane()
		{
			...
			var lanes = new[]
			{
				BoundedChannelFactory.Create<Envelope<int>>(capacity: 32, singleReader: true, singleWriter: true),
				BoundedChannelFactory.Create<Envelope<int>>(capacity: 32, singleReader: true, singleWriter: true),
				BoundedChannelFactory.Create<Envelope<int>>(capacity: 32, singleReader: true, singleWriter: true),
			};
			...
			var partition = new KeyPartitionNode<int>(
				"partition",
				input.Reader,
				lanes.Select(l => l.Writer).ToArray(),
				fatalErrorReporter: supervisor);

			var sinks = lanes
				.Select((lane, idx) => new CollectingSinkNode<int>($"sink-{idx}", lane.Reader, fatalErrorReporter: supervisor))
				.ToArray();
			...
			keyToLanes["a"].Count.ShouldBe(1);
			keyToLanes["b"].Count.ShouldBe(1);
			keyToLanes["c"].Count.ShouldBe(1);

			sinks.SelectMany(s => s.Items).Count().ShouldBe(keys.Length);
		}
	}
}
