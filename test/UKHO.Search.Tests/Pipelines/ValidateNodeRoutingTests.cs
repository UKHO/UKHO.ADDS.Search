...
namespace UKHO.Search.Tests.Pipelines
{
	public sealed class ValidateNodeRoutingTests
	{
		...
		public async Task Failed_messages_can_be_routed_to_error_output_without_leaking_to_main_output()
		{
			...
			var input = BoundedChannelFactory.Create<Envelope<int>>(capacity: 8, singleReader: true, singleWriter: true);
			var output = BoundedChannelFactory.Create<Envelope<int>>(capacity: 8, singleReader: true, singleWriter: true);
			var errors = BoundedChannelFactory.Create<Envelope<int>>(capacity: 8, singleReader: true, singleWriter: true);
			...
			sink.Items.Select(i => i.Payload).ToArray().ShouldBe(new[] { 2 });
			errorSink.Items.Count.ShouldBe(1);
			errorSink.Items[0].Status.ShouldBe(MessageStatus.Failed);
			errorSink.Items[0].Error.ShouldNotBeNull();
		...
		public async Task Already_failed_envelopes_are_forwarded_without_overwriting_error_details()
		{
			...
			var upstreamFailed = new Envelope<int>(string.Empty, 1)
				.MarkFailed(
					new PipelineError
					{
			...
			sink.Items[0].Status.ShouldBe(MessageStatus.Failed);
			sink.Items[0].Error.ShouldNotBeNull();
