	...
	public sealed class FatalErrorTests
	{
		...
		public async Task Unexpected_exception_faults_downstream_and_triggers_fail_fast_cancellation()
		{
			...
			var graph = HelloPipelineGraph.Create(
				...
				cts.Token,
				faultPipelineOnTransformException: true);

			await graph.Supervisor.StartAsync();

			await Should.ThrowAsync<InvalidOperationException>(
				async () => await graph.Supervisor.Completion.WaitAsync(cts.Token));
			...
			await Should.ThrowAsync<InvalidOperationException>(
				async () => await graph.Sinks[0].Completion.WaitAsync(cts.Token));
		}
	}
}
