...
namespace UKHO.Search.Tests.Pipelines
{
	public sealed class CompletionPropagationTests
	{
		...
		public async Task Completing_input_drains_pipeline_and_completes_downstream_cleanly()
		{
			...
			graph.Transforms.All(t => t.Completion.IsCompletedSuccessfully).ShouldBeTrue();
			graph.Sinks.All(s => s.Completion.IsCompletedSuccessfully).ShouldBeTrue();

			graph.Sinks.SelectMany(s => s.Items).Count().ShouldBe(50);
		}
	}
}
