...
namespace UKHO.Search.Tests.Pipelines
{
	public sealed class DeadLetterConcurrencyTests
	{
		...
		public async Task Multiple_dead_letter_sinks_can_append_to_the_same_file_safely()
		{
			...
			var filePath = Path.Combine(Path.GetTempPath(), "ukho-search", Guid.NewGuid().ToString("N"), "shared.jsonl");

			var supervisor = new PipelineSupervisor(cts.Token);
			var input1 = BoundedChannelFactory.Create<Envelope<int>>(capacity: 8, singleReader: true, singleWriter: true);
			var input2 = BoundedChannelFactory.Create<Envelope<int>>(capacity: 8, singleReader: true, singleWriter: true);

			var dl1 = new DeadLetterSinkNode<int>("dl-1", input1.Reader, filePath, fatalIfCannotPersist: true, fatalErrorReporter: supervisor);
			var dl2 = new DeadLetterSinkNode<int>("dl-2", input2.Reader, filePath, fatalIfCannotPersist: true, fatalErrorReporter: supervisor);
