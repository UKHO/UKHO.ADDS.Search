...
ORDER BY [Name] ASC, [Id] ASC;";
                ...
                _logger.LogInformation("Loaded {BusinessUnitCount} business units for checker selector.", businessUnits.Count);
                return BusinessUnitLookupResultDto.Success(businessUnits);
            }
            catch (SqlException ex)
            {
                _logger.LogError(ex, "DB error while loading business units for checker selector.");
                return BusinessUnitLookupResultDto.Failure("Database error while loading business units.");
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                _logger.LogError(ex, "Unexpected error while loading business units for checker selector.");
